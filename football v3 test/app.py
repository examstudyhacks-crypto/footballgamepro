...
def fixture_cards(fixtures: List[dict]) -> None:
    ...
    for match in fixtures[:12]:
        ...
        cards.append(
            ...
            f"<div class='fixture-vs'><span>{team_chip_html(match['home'])}</span><b class='vs-pill'>v</b><span class='away'>{team_chip_html(match['away'], align='right')}</span></div>"
            f"{group_html}"
            "</div>"
        )
...
def _live_feed(results: List[dict]) -> List[dict]:
    ...
    for r in results:
        ...
        feed.append({"minute": 45, "result_id": r["id"], "text": f"Half-time check-in: {r['home']} v {r['away']}", "type": "half_time"})
...
def live_broadcast(results: List[dict], seconds: int = 60) -> None:
    if not results:
        st.info("No live events to show.")
        return
    st.markdown("### 🎬 Live match centre")
    st.caption("Events reveal one at a time. Scores update as the goals come in, then the full result cards appear below.")
    event_box = st.empty()
    progress = st.progress(0)
    feed = _live_feed(results)
    scores = {r["id"]: {"home": 0, "away": 0} for r in results}
    result_lookup = {r["id"]: r for r in results}
    delay = max(0.35, min(2.4, seconds / max(1, len(feed))))
    for idx, ev in enumerate(feed, start=1):
        ...
        render_html(
            f"""
            <div class='live-card'>
                <div class='live-minute'>{h(label)}</div>
                <div class='live-event'>{h(ev['text'])}</div>
                {_score_tiles(scores, results)}
            </div>
            """
        )
        ...
        if idx < len(feed):
            time.sleep(delay)
    st.success("Live simulation complete. Results are below.")


def play_next(game: dict, live: bool, key_note: str = "") -> None:
    if game.get("stage") == "league":
        ...
        return
    if live:
        live_broadcast(results, seconds=60)
...
def action_buttons(game: dict, key_prefix: str, sticky: bool = False) -> None:
    if game.get("stage") not in ("league", "knockout"):
        return
    if sticky:
        render_html("<div class='mobile-action-bar'><b>Bottom menu</b><br><span>Watch live, quick sim, or show the latest result cards.</span></div>")
    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("🎬 Watch Live", type="primary", key=f"{key_prefix}_watch_live"):
            play_next(game, live=True, key_note=key_prefix)
    with c2:
        label = "⚡ Quick Sim"
        if game.get("stage") == "league":
        ...
        if st.button(label, key=f"{key_prefix}_quick_sim"):
            play_next(game, live=False, key_note=key_prefix)
    with c3:
...
def continue_button(game: dict, key: str) -> None:
    ...
    action_buttons(game, key_prefix=key)
    ...
    for idx, r in enumerate(results):
        ...
        render_html(
            ...
            <div class="result-card">
                <div class="scoreline">{team_chip_html(r['home'])} {r['home_goals']}–{r['away_goals']} {team_chip_html(r['away'], align='right')}</div>
...
def play_screen(game: dict) -> None:
    ...
    if game["stage"] == "league":
        ...
        action_buttons(game, key_prefix="top_next_matchday")
        st.caption("Watch Live reveals events over about 60 seconds. Quick Sim is instant. Neither option simulates ahead without another tap.")
        ...
        fixtures = upcoming_matches(game)
        fixture_cards(fixtures)
        action_buttons(game, key_prefix="top_next_knockout")
        st.caption("Watch Live reveals this round dramatically. Quick Sim is instant. The app pauses again before the next round.")
        if st.session_state.get("show_inline_results") and st.session_state.last_results:
            st.markdown("### Latest results")
            show_results(st.session_state.last_results)
    ...
    with tab_results:
        ...
        if game.get("stage") in ("league", "knockout"):
            ...
            action_buttons(game, key_prefix="results_continue")
    with tab_table:
        show_tables(game)
    with tab_bracket:
        show_bracket(game)
    ...
    if game.get("stage") in ("league", "knockout"):
        st.markdown("---")
        action_buttons(game, key_prefix="bottom_menu", sticky=True)
        if st.session_state.get("show_inline_results") and st.session_state.last_results:
            st.markdown("### Latest results")
            show_results(st.session_state.last_results)
